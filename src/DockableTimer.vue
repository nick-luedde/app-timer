<template>
  <section style="position: relative">
    <!-- Collapsed timer -->
    <p
      v-show="showHelp"
      class="box p-2 mb-3 is-size-7"
      style="position: absolute; top: -56px"
      role="alert"
    >
      Press [e] to expand
    </p>
    <div
      v-if="!expanded"
      ref="expandTimerActionEl"
      class="box mb-0 rounded p-3 has-background-white light-border"
      tabindex="0"
      title="Click to expand"
      :style="collapsedStyle"
      @click="handleExpand"
      @keydown="handleCollapsedKeydown"
      @focus="showHelp = true"
      @blur="showHelp = false"
    >
      <div class="has-text-centered pb-1 is-clickable">
        {{ displayTime.hr }}:{{ displayTime.min }}:{{
          String(displayTime.sec).padStart(2, "0")
        }}
      </div>
      <div class="buttons">
        <button
          class="button is-small is-rounded"
          :title="isTiming ? 'Pause' : 'Start'"
          :disabled="disabled"
          @click="isTiming ? handlePause($event) : handleStart($event)"
        >
          <span
            class="icon"
            :class="isTiming ? '' : 'has-text-success'"
          >
            <span
              aria-hidden="true"
              class="material-symbols-outlined"
            >{{
              isTiming ? "pause" : "play_arrow"
            }}</span>
          </span>
        </button>
        <button
          class="button is-small is-rounded"
          title="Stop"
          :disabled="disabled"
          @click="handleStop"
        >
          <span class="icon has-text-danger">
            <span
              aria-hidden="true"
              class="material-symbols-outlined"
            >stop</span>
          </span>
        </button>
      </div>
    </div>

    <!-- Expanded timer -->
    <div
      v-if="expanded"
      :style="expandedStyle"
      class="card"
    >
      <header class="card-header">
        <p class="card-header-title">
          <button
            class="button is-small is-white mr-2"
            title="Close"
            @click="handleClose"
          >
            <span class="icon">
              <span
                aria-hidden="true"
                class="material-symbols-outlined"
              >close</span>
            </span>
          </button>
          <a
            style="word-break: break-all;"
            href="#"
            @click.prevent="handleTitleClick"
          >
            [{{ title }}]
          </a>
        </p>
        <button
          class="card-header-icon"
          ref="collapseTimerActionEl"
          :title="expandLabel"
          @click="handleCollapse"
        >
          <span class="icon">
            <span
              aria-hidden="true"
              class="material-symbols-outlined"
            >{{
              expandIcon
            }}</span>
          </span>
        </button>
      </header>
      <div class="card-content">
        <div class="card-header-title is-size-7">
          <div class="content has-text-weight-bold is-flex-grow-1">
            <article class="is-flex is-flex-wrap-nowrap is-justify-content-center">
              <div class="p-4 light-border">
                {{ displayTime.hr }}
              </div>
              <div class="pt-4 pb-4 pr-2 pl-2">:</div>
              <div class="p-4 light-border">
                {{ displayTime.min }}
              </div>
              <div class="pt-4 pb-4 pr-2 pl-2">:</div>
              <div class="p-4 light-border">
                {{ String(displayTime.sec).padStart(2, "0") }}
              </div>
              <div class="pt-4 pb-4 pr-2 pl-2 is-hidden-touch">:</div>
              <div class="p-4 light-border is-hidden-touch">
                {{ String(displayTime.ms).padStart(3, "0") }}
              </div>
            </article>
          </div>
        </div>
      </div>
      <footer class="card-footer">
        <button
          class="button is-white card-footer-item"
          title="Start timer"
          :disabled="disabled"
          @click.prevent="handleStart"
        >
          <span class="icon-text">
            <span class="icon has-text-success">
              <span
                aria-hidden="true"
                class="material-symbols-outlined"
              >play_arrow</span>
            </span>
            <span class="is-hidden-touch"> Start </span>
          </span>
        </button>
        <button
          class="button is-white card-footer-item"
          title="Pause timer"
          :disabled="disabled"
          @click.prevent="handlePause"
        >
          <span class="icon-text">
            <span class="icon">
              <span
                aria-hidden="true"
                class="material-symbols-outlined"
              >pause</span>
            </span>
            <span class="is-hidden-touch"> Pause </span>
          </span>
        </button>
        <button
          class="button is-white card-footer-item"
          title="Stop timer"
          :disabled="disabled"
          @click.prevent="handleStop"
        >
          <span class="icon-text">
            <span class="icon has-text-danger">
              <span
                aria-hidden="true"
                class="material-symbols-outlined"
              >stop</span>
            </span>
            <span class="is-hidden-touch"> Stop </span>
          </span>
        </button>
      </footer>
    </div>
  </section>
</template>

<script setup>
export default {
  props: {
    title: String,
    expandedStyle: {
      type: Object,
      default: () => ({})
    },
    collapsedStyle: {
      type: Object,
      default: () => ({})
    },
    disabled: Boolean,
  },

  emits: ['start', 'pause', 'stop', 'close', 'title-click'],

  setup(props, ctx) {
    /** @type {VueOverrides} */
    const {
      ref,
      computed,
      nextTick
    } = Vue;

    const { local } = store;

    const timer = useTaskTimer();

    const collapseTimerActionEl = ref(null);
    const expandTimerActionEl = ref(null);

    const expanded = ref(local.timer.expanded.get());
    const showHelp = ref(false);

    const displayTime = computed(() => {
      //https://stackoverflow.com/questions/19700283/how-to-convert-time-in-milliseconds-to-hours-min-sec-format-in-javascript
      const totalms = timer.timing.ms;
      const ms = Math.floor(totalms % 1000);
      const sec = Math.floor((totalms / 1000) % 60);
      const min = Math.floor((totalms / 1000 / 60) % 60);
      const hr = Math.floor(totalms / 1000 / 60 / 60);

      return {
        ms,
        sec,
        min,
        hr,
        totalms
      };
    });

    const isTiming = computed(() => timer.timing.status === 'timing');
    const expandLabel = computed(() => props.expanded ? 'Collapse timer' : 'Expand timer');
    const expandIcon = computed(() => props.expanded ? 'expand_less' : 'expand_more');

    const setExpanded = (val) => {
      expanded.value = val;
      local.timer.expanded.set(val);
    };

    const handleClose = () => {
      timer.timer.reset();
      ctx.emit('close');
    };

    const handleStart = (/** @type {Event} */ e) => {
      e.stopPropagation();
      timer.timer.start({ withAnimation: true });
      ctx.emit('start');
    };

    const handlePause = (/** @type {Event} */ e) => {
      e.stopPropagation();
      const { hr, min, sec, ms, totalms } = displayTime.value;
      timer.timer.pause();
      ctx.emit('pause', { hr, min, sec, ms, totalms });
    };

    const handleStop = (/** @type {Event} */ e) => {
      e.stopPropagation();
      const { hr, min, sec, ms, totalms } = displayTime.value;
      timer.timer.stop();
      ctx.emit('stop', { hr, min, sec, ms, totalms });
    };

    const handleExpand = () => {
      setExpanded(true);
      nextTick().then(() =>
        requestAnimationFrame(() => collapseTimerActionEl.value.focus())
      );
    };

    const handleCollapse = () => {
      setExpanded(false);
      nextTick().then(() =>
        requestAnimationFrame(() => expandTimerActionEl.value.focus())
      );
    };

    const handleCollapsedKeydown = (/** @type {Event} */ e) => {
      if (e.code === "KeyE") handleExpand();
    };

    const handleTitleClick = () => ctx.emit('title-click');

    return {
      collapseTimerActionEl,
      expandTimerActionEl,
      expanded,
      showHelp,
      displayTime,
      isTiming,
      expandLabel,
      expandIcon,

      handleClose,
      handleStart,
      handlePause,
      handleStop,
      handleExpand,
      handleCollapse,
      handleCollapsedKeydown,
      handleTitleClick,

      local: {
        timer: store.local.timer,
      },
    };
  },

  data() { return {}; },

  template: document.querySelector("#task-timer-template").innerHTML,
};

app.component("task-timer", TaskTimer);
</script>